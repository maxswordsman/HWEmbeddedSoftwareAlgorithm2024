...
double w=0;
int dis[15005];
int op,num_new,head[200005],work[20005][2],flag,vis[15005],fa[15005][2];
int side[7005][3];
int channel[8005][100],ans_cha[80005],num_channel[80005];
vector<pair<int,int> > vec_side[10005];
//放大器
vector<int> vec_en[10005];
...
void add_side(int x,int y){
    ...
    while(1){
    	if(channel[fa_add[cha_new][h][1]][cha_new]==1){
    		...
    		num_new++;
    		add(side[i][0],side[i][1],side[i][2],M-1);
    		add(side[i][1],side[i][0],side[i][2],M-1);
            side[M-1][0]=side[i][0];
            side[M-1][1]=side[i][1];
            side[M-1][2]=side[i][2];
		}
