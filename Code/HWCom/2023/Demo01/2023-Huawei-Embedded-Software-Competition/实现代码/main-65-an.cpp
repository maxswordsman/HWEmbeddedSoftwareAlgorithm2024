...
/*void find_better_way(){
    ...
    for(int i=0;i<T;i++){
		...
		find(work[i][0],work[i][1]);
		vector<pair<int,int> > temp_side;
		int temp_cost=0;
		int h=work[i][1];
        while(1){
            temp_side.push_back({fa[cha_find][h][1],fa[cha_find][h][0]});
            temp_cost++;
            h=fa[cha_find][h][0];
            if(h==work[i][0]){
            	break;
        ...
        h=D;
        for(int t=temp_side.size()-1;t>=0;t--){
            if(h-side[temp_side[t].first][2]<0){
            	h=D;
            	temp_cost+=100;
			}
			h-=side[temp_side[t].first][2]; 
        }
			
		if(temp_cost>=cost[i]){
			for(int t=vec_side[i].size()-1;t>=0;t--){
        	 	channel[vec_side[i][t].first][ans_cha[i]]=1;
			}
			continue;
		} 
		vec_side[i].clear();
