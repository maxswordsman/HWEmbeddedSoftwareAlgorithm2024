/*
 * AdvanceV1版本 ---> 对应的Debug版本：AdvanceV1Debug.cpp
 *
 *  改进:
 *      1.在BaseLine的基础上，增加DFS算法
 *      2.对基础的BFS算法进行优化使得其可以遍历重 边情况
 * */
#include <iostream>
#include <vector>
#include <set>
...
public:
    ...
    bool isDie;
    bool quadraticProgramming;
    vector<int> pathNodes;

    // 构造函数
    Service(int id=0,int start = 0, int end = 0, int width = 0, int value = 0):
            id(id),start(start),end(end),width(width),value(value),
            edge_nums(ZERO),isDie(false){}
};
...
// 计算受到一个测试场景下规划成功的服务数量
pair<int, int> getServiceNum(Graph &graph,vector<int> &affected_services)
{
    ...
    return {successfulCnt1, failedCnt1};
}
/*********************************************其他函数*******************************************************/
